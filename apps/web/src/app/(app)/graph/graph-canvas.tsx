...
export function GraphCanvas({ engineConfigured }: { engineConfigured: boolean }) {
    ...
    <div className="grid lg:grid-cols-[1fr_320px] gap-6">
      <Card className="p-0 overflow-hidden relative min-h-[560px]">
        <div className="absolute top-4 left-4 right-4 z-10 flex items-center gap-3">
          <Input placeholder="Search nodes…" value={search} onChange={(e) => setSearch(e.target.value)} className="max-w-xs h-10 bg-bg/80 backdrop-blur" />
          <div className="ml-auto flex gap-2 flex-wrap text-xs text-fg-faint">
            {Object.entries(COLORS).map(([k, c]) => (<span key={k} className="flex items-center gap-1"><span className="size-2.5 rounded-full" style={{ background: c }} />{k}</span>))}
            <span>· solid = deterministic · dashed = AI</span>
          </div>
        </div>
        {!data && <p className="absolute inset-0 grid place-items-center text-fg-faint pulse-soft">Loading graph…</p>}
        {data && data.nodes.length === 0 && (
          <div className="absolute inset-0 grid place-items-center text-center px-8">
            <div className="space-y-2">
              <p className="text-2xl font-title">{offline ? "Graph engine offline" : "Your graph is empty"}</p>
              <CardHint>{offline ? "Notes you add are stored safely and will be indexed once the engine is up." : "Add a note above or run a task — outputs become nodes."}</CardHint>
              {data.nodes.length === 0 && <p className="num text-xs text-fg-faint">engine: {data.engine}</p>}
            </div>
          </div>
        )}
      ...
      <Card className="space-y-4 min-h-[200px]">
        {!selected ? (
          <>
            ...
            <CardHint>Click a node to see its content and connections. Drag to pan, wheel to zoom.</CardHint>
            {data && <p className="text-xs text-fg-faint"><span className="num">{data.nodes.length}</span> nodes · <span className="num">{data.edges.length}</span> edges</p>}
          </>
        ) : (
          <>
            ...
            <div className="flex gap-2"><Badge>{selected.node_type}</Badge><Badge>deg <span className="num">{selected.degree}</span></Badge></div>
            {!detail && <p className="text-fg-faint text-sm pulse-soft">Loading…</p>}
            {detail?.error && <p className="text-red text-sm">{detail.error}</p>}
            ...
            {detail?.connections && detail.connections.length > 0 && (
              <div className="space-y-1">
                <p className="text-xs uppercase tracking-wide text-fg-faint">Connections</p>
                <ul className="text-sm space-y-1">
                  {detail.connections.map((c) => (
                    <li key={c.id} className="flex gap-2"><span className="text-fg-faint num text-xs">{c.relation}{c.origin === "ai" ? " ·ai" : ""}</span><span className="truncate">{c.title}</span></li>
                  ))}
                </ul>
              </div>
            )}
