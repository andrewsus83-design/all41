...
export function ResultPanel({ result, billedUsd, modelsUsed, title }: { result: TaskResult; billedUsd: number | null; modelsUsed?: string[]; title?: string }) {
  ...
  // SEO and Proposal have dedicated detail pages; Clip and Web Builder render fully inline (no separate page).
  const detailHref = runId && !isClip && !isWebsite ? (isProposal ? `/my-apps/proposal/${runId}` : `/my-apps/audit/${runId}`) : null;
  return (
