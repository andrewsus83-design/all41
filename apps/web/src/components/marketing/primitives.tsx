...
export function Eyebrow({ phase = "neutral", children }: { phase?: Phase; children: ReactNode }) {
  return (
    <p className="flex items-center gap-2 text-xs uppercase tracking-[0.18em] text-fg-muted font-medium">
      <span className={cn("inline-block w-2 h-2 rounded-full", dot[phase])} aria-hidden />
      {children}
    </p>
  );
}
...
export function H2({ className, ...props }: HTMLAttributes<HTMLHeadingElement>) {
  return <h2 className={cn("text-3xl md:text-5xl font-semibold leading-[1.08] max-w-3xl", className)} {...props} />;
}

export function Lead({ className, ...props }: HTMLAttributes<HTMLParagraphElement>) {
  return <p className={cn("text-lg md:text-xl text-fg-muted max-w-2xl leading-relaxed", className)} {...props} />;
}
