  ...
  return (
    <header className="sticky top-3 md:top-5 z-40 px-4 sm:px-0">
      <div className="mx-auto w-full sm:w-[82%] max-w-[1400px]">
        ...
        <div className="relative grad-nav text-white rounded-full shadow-lift flex items-center justify-between gap-4 h-14 md:h-16 pl-5 md:pl-7 pr-2 md:pr-2.5">
          ...
          <nav className="hidden lg:flex items-center gap-0.5 absolute left-1/2 -translate-x-1/2" aria-label="Primary">
            {NAV.map((n) => (
              <Link key={n.href} href={n.href} className="rounded-full px-3.5 py-2 text-sm text-white/85 hover:text-white hover:bg-white/15 transition">
                {n.label}
              </Link>
            ))}
          </nav>
          <div className="flex items-center gap-1.5 shrink-0">
            <Link href={signedIn ? "/my-apps" : "/login"} className="hidden lg:inline-flex items-center rounded-full px-3.5 py-2 text-sm text-white/85 hover:text-white hover:bg-white/15 transition">
