...
/** Full-screen app detail: a locked one-page cover (Use now · See samples), then a swipeable
 * gallery of finished, real examples (canned demo data, nothing charged). Header + ✕ stay pinned. */
export function AppDetail({ app, onClose, onBuild }: { app: CatalogApp; onClose: () => void; onBuild: () => void }) {
  ...
  const closeBtn = (fn: () => void) => (
    <button type="button" onClick={fn} aria-label="Close" className="size-12 rounded-full grid place-items-center text-fg-muted hover:text-fg bg-bg-elev border border-line hover:bg-bg-elev-2 transition text-xl shrink-0">✕</button>
  );
    ...
    return (
      <div className="fixed inset-0 z-[88] bg-bg flex flex-col scene-in" role="dialog" aria-modal="true" aria-label={`${app.name} samples`}>
        ...
        <div
          ...
          onTouchEnd={(e) => { if (touchX.current == null) return; const dx = e.changedTouches[0].clientX - touchX.current; if (Math.abs(dx) > 70) go(dx < 0 ? 1 : -1); touchX.current = null; }}
        >
